...
Сценарий: 02.18. [КП1: ПЗ -> Договор] Возврат поставщику(ДД4)
	...
	* Создаем возврат поставщику
		...
		И в таблице 'СписокДокументыПоступления' я перехожу к строке:
			| "Контрагент" | "Организация"              | "Поставщик"  | "Склад"  | "Сумма"        | "Хоз. операция"        |
			| "Поставщик4" | "Королевство Севера [223]" | "Поставщик4" | "Склад1" | "1 020 000,00" | "Закупка у поставщика" |
...
Сценарий: 02.21. [КП2: ПЗ -> Заказ] ПТУ(ДД5)
	...
	* Создаем ПТУ по заказу
		...
		Тогда открылось окно "Заказы поставщикам"
		И в таблице 'Список' я перехожу к строке:
			| "Поставщик"  | "Сумма"        | "Хоз. операция"        |
			| "Поставщик5" | "1 020 000,00" | "Закупка у поставщика" |
